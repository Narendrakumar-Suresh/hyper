...
export function useCourses() {
  ...
  useEffect(() => {
    // Check if auth is still loading. If so, don't proceed with fetching.
    if (authLoading) {
      return;
    }
    
    // Check for authentication. If not authenticated or no user ID, stop loading.
  ...
  }, [user?.id, isAuthenticated, authLoading]); // Dependency array is fine
