...
export default function ClientSchoolLearnerView({ slug }: { slug: string }) {
    ...
    const fetchSchool = async () => {
      ...
      try {
        ...
        const schoolResponse = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/organizations/slug/${slug}`
        );
        ...
        const isOwnerOrAdmin = schools?.some(
          (s) =>
        ...
        if (isOwnerOrAdmin) {
          const allCohortsResponse = await fetch(
            `${process.env.NEXT_PUBLIC_BACKEND_URL}/cohorts/?org_id=${transformedSchool.id}`
          );
          ...
          const userCohortsResponse = await fetch(
            `${process.env.NEXT_PUBLIC_BACKEND_URL}/users/${user.id}/org/${transformedSchool.id}/cohorts`
          );
  ...
  const fetchCohortCourses = async (cohortId: number) => {
    ...
    try {
      ...
      const cohortUrl = activeCohort?.joined_at
        ? `${url}&joined_at=${activeCohort.joined_at}`
        : url;
  ...
  const fetchCompletionData = async (cohortId: number, userId: string) => {
    ...
    try {
      const { taskCompletions, questionCompletions } = await getCompletionData(
        cohortId,
        userId
      );
  ...
  const handleCourseSelect = (index: number) => {
    ...
    const modules = transformCourseToModules(
      courses[index],
      activeCohort?.joined_at
    );
    setCourseModules(modules);
  ...
  return (
    <>
      ...
      {isAdminOrOwner && (
        <div className="bg-[#111111] border-b border-gray-800 text-white py-3 px-4 text-center shadow-sm">
          <p className="font-light text-sm">
            You are viewing all the cohorts in this school because you are an
            admin. Learners only see the cohorts they are enrolled in.
          </p>
        </div>
      )}
      ...
      <div className="hidden sm:block">
        ...
        />
        <div className="flex justify-end px-4 py-2 bg-black">
          <Button asChild className="bg-amber-50">
            <Link href="https://sensai-eight-ruby.vercel.app/" target="_blank">
              Forum
            </Link>
          </Button>
        </div>
              ...
              <div className="pt-24 px-4">
                <div className="flex flex-col items-center justify-center py-12 rounded-lg text-center">
                  ...
                  <p className="text-gray-400">
                    You are not enrolled in any cohorts for this school
                  </p>
                </div>
              </div>
            )}
                  ...
                  <div className="w-full">
                    ...
                    {courses.length === 0 ? (
                      <div className="pt-12 text-center px-4">
                        ...
                        <p className="text-gray-400">
                          There are no courses in this cohort yet
                        </p>
                      </div>
                    ) : (
