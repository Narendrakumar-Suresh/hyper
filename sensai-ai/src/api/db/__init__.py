...
async def create_organizations_table(cursor):
    ...
    await cursor.execute(f"CREATE INDEX idx_org_slug ON {organizations_table_name} (slug)")
...
async def create_org_api_keys_table(cursor):
    ...
    await cursor.execute(f"CREATE INDEX idx_org_api_key_org_id ON {org_api_keys_table_name} (org_id)")
    await cursor.execute(f"CREATE INDEX idx_org_api_key_hashed_key ON {org_api_keys_table_name} (hashed_key)")
...
async def create_cohort_tables(cursor):
    ...
    await cursor.execute(f"CREATE INDEX idx_cohort_org_id ON {cohorts_table_name} (org_id)")
...
async def create_courses_table(cursor):
    ...
    await cursor.execute(f"CREATE INDEX idx_course_org_id ON {courses_table_name} (org_id)")
...
async def create_course_cohorts_table(cursor):
    ...
    await cursor.execute(f"CREATE INDEX idx_course_cohort_course_id ON {course_cohorts_table_name} (course_id)")
    await cursor.execute(f"CREATE INDEX idx_course_cohort_cohort_id ON {course_cohorts_table_name} (cohort_id)")
...
async def create_milestones_table(cursor):
    ...
    await cursor.execute(f"CREATE INDEX idx_milestone_org_id ON {milestones_table_name} (org_id)")
...
async def create_course_milestones_table(cursor):
    ...
    await cursor.execute(f"CREATE INDEX idx_course_milestone_course_id ON {course_milestones_table_name} (course_id)")
    await cursor.execute(f"CREATE INDEX idx_course_milestone_milestone_id ON {course_milestones_table_name} (milestone_id)")
...
async def create_tasks_table(cursor):
    ...
    await cursor.execute(f"CREATE INDEX idx_task_org_id ON {tasks_table_name} (org_id)")
...
async def create_questions_table(cursor):
    ...
    await cursor.execute(f"CREATE INDEX idx_question_task_id ON {questions_table_name} (task_id)")
...
async def create_course_tasks_table(cursor):
    ...
    await cursor.execute(f"CREATE INDEX idx_course_task_task_id ON {course_tasks_table_name} (task_id)")
    await cursor.execute(f"CREATE INDEX idx_course_task_course_id ON {course_tasks_table_name} (course_id)")
    await cursor.execute(f"CREATE INDEX idx_course_task_milestone_id ON {course_tasks_table_name} (milestone_id)")
...
async def create_scorecards_table(cursor):
    ...
    await cursor.execute(f"CREATE INDEX idx_scorecard_org_id ON {scorecards_table_name} (org_id)")
...
async def create_question_scorecards_table(cursor):
    ...
    await cursor.execute(f"CREATE INDEX idx_question_scorecard_question_id ON {question_scorecards_table_name} (question_id)")
    await cursor.execute(f"CREATE INDEX idx_question_scorecard_scorecard_id ON {question_scorecards_table_name} (scorecard_id)")
...
async def create_chat_history_table(cursor):
    ...
    await cursor.execute(f"CREATE INDEX idx_chat_history_user_id ON {chat_history_table_name} (user_id)")
    await cursor.execute(f"CREATE INDEX idx_chat_history_question_id ON {chat_history_table_name} (question_id)")
...
async def create_task_completion_table(cursor):
    ...
    await cursor.execute(f"CREATE INDEX idx_task_completion_user_id ON {task_completions_table_name} (user_id)")
    await cursor.execute(f"CREATE INDEX idx_task_completion_task_id ON {task_completions_table_name} (task_id)")
    await cursor.execute(f"CREATE INDEX idx_task_completion_question_id ON {task_completions_table_name} (question_id)")
...
async def create_code_drafts_table(cursor):
    ...
    await cursor.execute(f"CREATE INDEX idx_code_drafts_user_id ON {code_drafts_table_name} (user_id)")
    await cursor.execute(f"CREATE INDEX idx_code_drafts_question_id ON {code_drafts_table_name} (question_id)")
...
async def create_course_generation_jobs_table(cursor):
    ...
    await cursor.execute(f"CREATE INDEX idx_course_generation_job_course_id ON {course_generation_jobs_table_name} (course_id)")
...
async def create_task_generation_jobs_table(cursor):
    ...
    await cursor.execute(f"CREATE INDEX idx_task_generation_job_task_id ON {task_generation_jobs_table_name} (task_id)")
    await cursor.execute(f"CREATE INDEX idx_task_generation_job_course_id ON {task_generation_jobs_table_name} (course_id)")
...
async def init_db():
    ...
    async with get_new_db_connection() as conn:
        ...
        for table_name, create_fn in table_creators:
            if not await check_table_exists(table_name, cursor):
                await create_fn(cursor)
...
async def delete_useless_tables():
    ...
    async with get_new_db_connection() as conn:
        ...
        # Add new columns to course_cohorts if missing
        ...
        for col, typ, default in [
            ...
            if col not in cols:
                ...
                await cursor.execute(f"ALTER TABLE {course_cohorts_table_name} ADD COLUMN {col} {typ}{dflt}")
